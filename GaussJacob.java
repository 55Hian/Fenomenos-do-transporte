 ...
 */
package exemplogaussjacob;

/**
 *
 * @author hian.adamaceno
 */
public class GaussJacob {
   ...
   public GaussJacob(double[][] matrizExtendida, double[] chuteInicial){
   ...
   private double[][] copiaMatriz(double[][] origem){
       ...
       for(int nl = 0; nl < origem.length; nl++ ){
            for(int nc = 0; nc < origem[0].length; nc++){
   ...
   private double calculaLinha(int linha){
       ...
       for (int nc = 0; nc < this.C.length; nc++){
           temp -= this.C[linha][nc]*this.Xo[nc];
